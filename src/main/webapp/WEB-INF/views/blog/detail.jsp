...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="row">
                ...
                action을 가지지 않습니다. 그리고 제축버튼도 제출기능을 막고 fetch 요청만 넣습니다. -->
                    <div class="col-2">
    ...
    <script>
        ...
        function getAllreplies(id){
            ...
            let str = ""; // 받아온 json을 표현할 html 코드를 저장할 문자열 str선언
        ...
        function insertReply(){
            ...
            fetch(url, {
                method:'post',
                headers: { // headers에는 보내는 데이터의 자료형에 대해서 기술
        ...
        function openUpdateReplyModal(){
            ...
            // 위에서 추출한 id번호를 이용해 document.querySelector()를 통해 요소를 가져온 다음
            // 해당 요소의 text값을 얻어서 모달창의 폼 양식 내부에 넣어줍니다.
            // 위에 부여한 id를 이용해 span태그를 가지고 오는 코드
            // 태그는 제거하고 내부 문자만 가지고 오도록 처리하는 코드
            let replyWriter = document.querySelector(replyWriterId).innerText;
            let replyContent = document.querySelector(replyContentId).innerText;
            ...
            $modalReplyWriter.value = replyWriter;
            $modalReplyContent.value = replyContent;
        ...
        function deleteReply(){
            ...
            if(confirm("정말로 삭제하겠어요?")){ // 예, 아니오로 답 할수 있는 경고창을 띄웁니다.
                // 예를 선택하면 true, 아니오를 선택하면 false입니다.
                
                // 위 정보를 토대로 url 세팅 후 비동기 요청으로 삭제를 처리하고 댓글을 갱신해주세요.
                let url = `/reply/\${replyId}`; // 상대주소 /reply/\${replyId}/
                fetch(url, {
                    method:'delete'})
                .then(() => {
