...
public class ReplyController {
    ...
    // http://localhost:8080/reply/2/all => blogId 파라미터에 2가 전달된 것으로 간주한다.
    ...
    public ResponseEntity<List<Reply>> findAllReplies(@PathVariable long blogId) {
        ...
        return ResponseEntity
                .ok() // 200 코드, 상태 코드와 body에 전송할 데이터를 같이 작성할수도 있음.
                .body(replies); // 리플목록
    }

    // replyId를 주소에 포함시켜서 요청하면 해당 번호 댓글 정보를 json으로 리턴하는 메서드
    // 예시) /replyId/5 -> replyId 변수에 5가 대입되도록 주소 설정 및 메서드 선언
    ...
    public ResponseEntity<?> findByReplyId(@PathVariable long replyId) {
        ...
        Reply reply = (Reply) replyService.findByReplyId(replyId);
            if (reply == null) {
                try {
    ...
    // post 방식으로 /reply 주소로 요청이 들어왔을때 실행되는 메서드 insertReply()를 작성해주세요.
    @RequestMapping(value = "", method = RequestMethod.POST)
        // Rest컨트롤러는 데이터를 json으로 주고받음. 따라서 @RequestBody를 이용해 json으로 들어온 데이터를 억직렬화 하도록 설정
    public ResponseEntity<String> insertReply(@RequestBody Reply reply){
        System.out.println("데이터 들어오는지 확인 : " + reply);
        replyService.save(reply);
        return ResponseEntity
                .ok("댓글 입력이 잘되었다.");
    }

    // delete 방식으로 /reply/{번호} 주소로 요청이 들어왔을때 실행되는 메서드 deleteReply()를 작성해주세요.
    ...
    // 수정로직은 put, pacth 메서드로 /reply/댓글번호 주소로
    // ReplyUpdateRequestDTO를 requestBody로 받아 요청 처리를 하게 만들어주세요.
