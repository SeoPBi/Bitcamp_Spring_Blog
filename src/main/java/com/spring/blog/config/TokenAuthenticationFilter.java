...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    // 토큰제공자 내부의 vaildToken메서드를 이용해서 토큰의 유효성을 검증할 예정이므로 의존성 주입
    private final TokenProvider tokenProvider;

    // 토큰은 RequestHeader에 포함되서 들어오므로, 해당 요소를 얻어낼 수 있도록 명칭 상수화
    private final static String HEADER_AUTHORIZATION = "Authorization";
    // 토큰기반 입증시 실제 토큰값 앞에 접두어로 붙는 Bearer 제거를 위한 상수
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // 토큰은 요청 헤더를 이용해서 제출되기 때문에, request에서 header를 얻어와야 합니다.
    ...
    private String getAccessToken(String authorizationHeader) {
        ...
        if(authorizationHeader != null && authorizationHeader.startsWith(TOKEN_PREFIX)){
            return authorizationHeader.substring(TOKEN_PREFIX.length()); //앞 7글자(bearer )를 소거한 값만
        }
        return null;
    }
}
