    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .formLogin(formLoginConfig -> {
                    formLoginConfig
                            // .loginPage("/login") // 폼에서 날려준 정보를 토대로 로그인 처리르  해주는 주소(post)
                            // .defaultSuccessUrl("/blog/list");
                            .disable(); // 토큰 기반 로그인시에는 폼 로그인을 막아야 합니다.
                })
                .logout(logoutConfig -> {
                    logoutConfig
                            .logoutUrl("/logout") // 디폴트로 "/logout"으로 잡아주기 때문에 굳이 설정할 필요 없음
...
//                .defaultSuccessUrl("/blog/list") // 로그인 화면 처음으로 보여질 페이지
//                .and()
//                .logout() // 로그아웃 관련 설정
//                .logoutSuccessUrl("/login") // 로그아웃 성공했으면 넘어갈 경로
//                .invalidateHttpSession(true)
//                .and()// 로그아웃하면 다음 접속 시 로그인이 풀려있게 설정
//                .csrf() // csrf 공격 방지용 토큰
//                .disable() // 을 쓰지 않겠음.
//                .build();
    }

    // 위에 설정을 따라가는 인증은 어떤 서비스를 통해서 설정할 것 인가?
    @Bean
