...
public class ReplyRepositoryTest {
    ...
    @Test
    @Transactional
    @DisplayName("2번 글에 연동된 댓글번호 2번을 삭제한 다음, 전체 데이터 개수가 3개이고, 그리고 2번으로 재 조회시 0일 것이다.")
    public void deleteByReplyIdTest(){
        ...
        // then : 2번글에 연동된 댓글 개수는 3개일 것이고, 2번 댓글 재 조회 시 null
    ...
    @Test
    @Transactional
    @DisplayName("픽스처를 이용해 INSERT후, 전체 데이터를 가져와서 마지막 인덱스 번호 요소를 얻어와서 입력했던 fixture와 비교하면 같다.")
    public void saveTest() {
        ...
        //then : blogId번 글의 전체 댓글을 가지고 오고나서 마지막 인덱스 요소만 변수에 저장한 다음
    ...
    public void updateTest() {
        // given :
        String replyWriter = "수정할 댓글쓴이";
        String replyContent = "수정할 댓글내용";
        long replyId = 3;
    ...
    public void deleteBlogIdTest() {
        ...
        // then : blogId번 글 전체 댓글을 얻어와서 null인지 확인
        List<ReplyResponseDTO> resultList = replyRepository.findAllByBlogId(blogId);
