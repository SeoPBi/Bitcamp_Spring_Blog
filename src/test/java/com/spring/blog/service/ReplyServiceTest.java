...
public class ReplyServiceTest {
    ...
    // findAllByBlogIdTest()는 RepositoryTest코드를 참조해서 작성해주세요.
    @Test
    @Transactional
    ...
    @Test
    @Transactional
    @DisplayName("댓글번호 5번으로 얻어온 객체의 replyWriter은 개발고수이고, replyId는 5일 것이다.")
    ...
    @Test
    @Transactional
    @DisplayName("댓글번호 3번 삭제시, 2번 포스팅의 댓글은 3개이고, 3번으로 재요청시 null")
    ...
    @Test
    @Transactional
    @DisplayName("replyWriter는 토토비, replyContent는 토비보자,blogId는 3번으로 입력시 3번글 연동댓글 2개, 입력된 픽스처와 일치하는 멤버 변수")
    ...
    @Test
    @Transactional
    @DisplayName("replyId 2번의 replyWriter를 바뀐 짹짹이, replyContent를 부엉부엉부엉부엉으로, updateAt이 publishedAt보다 이후로 바뀜")
